...
impl<R: tauri::Runtime> LocationConnectivityState<R> {
    ...
    pub async fn update_location_status(&self) -> Result<(), LocationConnectivityError> {
        ...
        *current_status = new_status.clone();
        ...
        if status_changed {
            ...
            self.emit_location_event(LocationEventType::LocationChanged, &new_status)
                .await;
        }
    ...
    pub async fn emit_location_event(
        &self,
    ...
    async fn emit_location_event_with_retry(&self, event: &LocationEvent) {
        ...
        for attempt in 0..MAX_RETRIES {
            match self
                .app_handle
                .emit("location-connectivity://location-changed", event)
            {
                Ok(()) => {
                    if attempt > 0 {
                        tracing::debug!(
                            "Successfully emitted location event after {} retries",
                            attempt
                        );
                    }
                    return;
                ...
                Err(e) => {
                    if attempt < MAX_RETRIES - 1 {
                        tracing::warn!(
                            "Failed to emit location event (attempt {}/{}): {}. Retrying...",
                            attempt + 1,
                            MAX_RETRIES,
                            e
                        );
                        ...
                        tracing::error!(
                            "Failed to emit location event after {} attempts: {}",
                            MAX_RETRIES,
                            e
                        );
                    }
