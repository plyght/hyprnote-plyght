...
export const commands = {
  ...
  async windowPosition(window: HyprWindow, pos: KnownPosition): Promise<null> {
    return await TAURI_INVOKE("plugin:windows|window_position", {
      window,
      pos,
    });
  },
  async windowResizeDefault(window: HyprWindow): Promise<null> {
    return await TAURI_INVOKE("plugin:windows|window_resize_default", {
      window,
    });
  },
  ...
  async windowSetFloating(window: HyprWindow, v: boolean): Promise<null> {
    return await TAURI_INVOKE("plugin:windows|window_set_floating", {
      window,
      v,
    });
  },
  async windowNavigate(window: HyprWindow, path: string): Promise<null> {
    return await TAURI_INVOKE("plugin:windows|window_navigate", {
      window,
      path,
    });
  },
  async windowEmitNavigate(window: HyprWindow, path: string): Promise<null> {
    return await TAURI_INVOKE("plugin:windows|window_emit_navigate", {
      window,
      path,
    });
  },
  ...
  async windowSetOverlayBounds(
    name: string,
    bounds: OverlayBound,
  ): Promise<null> {
    return await TAURI_INVOKE("plugin:windows|window_set_overlay_bounds", {
      name,
      bounds,
    });
  },
  async windowRemoveOverlayBounds(name: string): Promise<null> {
    return await TAURI_INVOKE("plugin:windows|window_remove_overlay_bounds", {
      name,
    });
  },
  async setFakeWindowBounds(name: string, bounds: OverlayBound): Promise<null> {
    return await TAURI_INVOKE("plugin:windows|set_fake_window_bounds", {
      name,
      bounds,
    });
  },
